...
export default function Preview() {
  ...
  return (
    <div className='flex items-center justify-center relative w-full lg:flex-row flex-col'>
          ...
          <div className='w-full h-full overflow-hidden'>
            <Gravity gravity={{ x: 0, y: 1 }}>
              {SKILL[activeTab].icons.map((icon, index) => {
                const randomY = Math.random() * 10 + 5 // 隨機生成一個 y 軸位置
                const randomX = Math.random() * 10 + 5 // 隨機生成一個 x 軸位置
                return (
                  <MatterBody
                    key={index}
                    matterBodyOptions={{
